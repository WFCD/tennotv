        ...
        contentCreators.forEach(creator => {
          $($('#creators').find('.row.text-center')[0]).append(`
            <div class="col-sm-2" id='creator-${creator.id}'>
              <figure>
                <a href="/${creator.account_name.replace(/\s/ig, '').toLowerCase()}"  rel="noopener" title="${creator.account_name}" alt="${creator.account_name}'s Page"><img src="${creator.youtube_thumbnail}" height="75px" />
